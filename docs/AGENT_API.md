...
| `status` | `BACKLOG`, `REFINING`, `TODO`, `IN_PROGRESS`, `READY_FOR_REVIEW`, `IN_REVIEW`, `APPROVED`, `FIX_NEEDED`, `FIX_IN_PROGRESS`, `RE_REVIEW`, `PENDING_DECISION`, `DONE`, `CANCELLED`, `FAILED` (canonical workflow order); must be enabled for the project. If omitted during creation, the project's `defaultStatus` is used. New orchestration statuses are opt-in and are not enabled on existing or new projects by default. |
