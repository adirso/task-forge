...
export class MysqlMonitorStore implements MonitorStore {
  ...
  async acquireLease(runId: string, ownerId: string, now: string, expiresAt: string) { await this.pool.execute("INSERT INTO delivery_monitor_leases (run_id,owner_id,acquired_at,expires_at) VALUES (?,?,?,?) ON DUPLICATE KEY UPDATE owner_id=IF(expires_at <= VALUES(acquired_at), VALUES(owner_id), owner_id), acquired_at=IF(expires_at <= VALUES(acquired_at), VALUES(acquired_at), acquired_at), expires_at=IF(expires_at <= VALUES(acquired_at), VALUES(expires_at), expires_at)",[runId,ownerId,now,expiresAt]); const [rows] = await this.pool.execute("SELECT owner_id ownerId, acquired_at acquiredAt, expires_at expiresAt FROM delivery_monitor_leases WHERE run_id=?", [runId]); const current = (rows as Array<{ownerId:string; acquiredAt:string; expiresAt:string}>)[0]; return current?.ownerId === ownerId ? { runId,ownerId,acquiredAt:current.acquiredAt,expiresAt:current.expiresAt } : null; }
