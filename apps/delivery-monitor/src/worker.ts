...
export type MonitorWorkerOptions = { store: MonitorStore; ownerId: string; config: Pick<DeliveryMonitorConfig, "pollIntervalMs" | "batchSize" | "leaseDurationMs" | "maxRetries">; list: (cursor: string | null, limit: number) => Promise<MonitorItem[]>; poll: (item: MonitorItem, checkpoint: MonitorCheckpoint | null) => Promise<PollResult>; onError?: (error: unknown) => void };
