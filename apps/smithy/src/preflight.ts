...
import { executeCommand, providerEnvironment, renderCommand, type CommandResult } from "./command.js";
import { redact } from "./security.js";
import { DISABLED_SANDBOX_POLICY, type SandboxPolicy } from "./sandbox.js";
...
function diagnostic(result: CommandResult): { status: ProviderHealthStatus; message: string } {
  ...
  if (result.policyViolation) return { status: "FAILED", message: `Provider sandbox policy check failed (${result.policyViolation}): ${redact((result.error?.message ?? output) || "execution was stopped").slice(0, 240)}` };
...
export async function checkProvider(provider: ProviderLabel, config: ProviderConfig, execute = executeCommand, now = () => new Date().toISOString(), sandbox: SandboxPolicy = DISABLED_SANDBOX_POLICY): Promise<ProviderHealth> {
  let command: string;
  try {
    const executable = renderCommand(config.cmd, "").executable;
    command = config.healthCmd?.trim() || `${executable} --version`;
    renderCommand(command, "");
    const result = await execute(command, "", config.repo || process.cwd(), 10_000, undefined, undefined, providerEnvironment(process.env, undefined, sandbox.environmentAllow), sandbox);
...
export async function runProviderPreflight(providers: Partial<Record<ProviderLabel, ProviderConfig>>, execute = executeCommand, sandbox: SandboxPolicy = DISABLED_SANDBOX_POLICY): Promise<ProviderHealth[]> {
  return Promise.all(Object.entries(providers).map(([provider, config]) => config ? checkProvider(provider, config, execute, undefined, sandbox) : Promise.resolve({ provider, status: "FAILED" as const, message: "Provider configuration is empty.", checkedAt: new Date().toISOString() })));
}
