...
export interface JobStore { accept(eventId: string, provider: string, taskId: string, body: string): { job: Job; duplicate: boolean }; setRunId(eventId: string, runId: string): void; markRunning(eventId: string): void; markComplete(eventId: string, status: "SUCCEEDED" | "FAILED"): void; pending(): Job[]; close?(): void; }
...
export class SqliteJobStore implements JobStore {
  ...
  constructor(path = process.env.SMITHY_DB_PATH ?? "./data/smithy.sqlite") {
    ...
    this.db.exec("CREATE TABLE IF NOT EXISTS smithy_jobs (event_id TEXT PRIMARY KEY, provider TEXT NOT NULL, task_id TEXT NOT NULL, body TEXT NOT NULL, run_id TEXT, status TEXT NOT NULL CHECK(status IN ('PENDING','RUNNING','SUCCEEDED','FAILED')), created_at TEXT NOT NULL, updated_at TEXT NOT NULL)");
