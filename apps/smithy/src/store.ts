...
export interface JobStore { accept(eventId: string, provider: string, taskId: string, body: string): { job: Job; duplicate: boolean }; setRunId(eventId: string, runId: string): void; markRunning(eventId: string): void; markComplete(eventId: string, status: "SUCCEEDED" | "FAILED" | "CANCELLED"): void; requeue(eventId: string, staleBefore?: string): boolean; cancel(eventId: string): boolean; pending(): Job[]; close?(): void; }
