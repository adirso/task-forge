...
export async function projectRoutes(app: FastifyInstance) {
  ...
  app.post("/", { schema: { tags: ["Projects"], summary: "Create a project" } }, async (request, reply) => {
    ...
    try {
      await db.transaction(async () => {
        await db.prepare(`INSERT INTO projects (id, \`key\`, name, description, repo_url, color, owner_id, created_at, updated_at)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
          .run(id, body.key, body.name, body.description, body.repoUrl ?? null, body.color, request.authUser.id, now, now);
        await db.prepare("INSERT INTO project_members (project_id, user_id, role, created_at) VALUES (?, ?, 'OWNER', ?)")
          .run(id, request.authUser.id, now);
        await db.prepare("INSERT INTO phases (id, project_id, number, goal, is_active, created_at, updated_at) VALUES (?, ?, 1, ?, 1, ?, ?)")
          .run(randomUUID(), id, "Plan and deliver the first project milestone.", now, now);
      })();
    } catch (error) {
      if (/UNIQUE|Duplicate entry/i.test(String(error))) return reply.code(409).send({ error: "Project key already exists" });
      throw error;
    }
  ...
  app.get<{ Params: ProjectParams }>("/:id", { schema: { tags: ["Projects"], summary: "Get a project" } }, async (request, reply) => {
    if (!(await requireProjectAccess(request, reply, request.params.id))) return;
    const row = await db.prepare("SELECT * FROM projects WHERE id = ?").get(request.params.id) as Record<string, unknown> | undefined;
  ...
  app.post<{ Params: ProjectParams }>("/:id/members", { schema: { tags: ["Projects"], summary: "Add a project member" } }, async (request, reply) => {
    if (!(await requireProjectOwnerOrAdmin(request, reply, request.params.id))) return;
    const body = memberAddSchema.parse(request.body);
    const user = await db.prepare("SELECT id FROM users WHERE id = ?").get(body.userId);
    if (!user) return reply.code(404).send({ error: "User not found" });
    const project = await db.prepare("SELECT owner_id FROM projects WHERE id = ?").get(request.params.id) as { owner_id: string };
    ...
    await db.prepare(`INSERT INTO project_members (project_id, user_id, role, created_at) VALUES (?, ?, ?, ?)
  ...
  app.delete<{ Params: ProjectParams & { userId: string } }>("/:id/members/:userId", { schema: { tags: ["Projects"], summary: "Remove a project member" } }, async (request, reply) => {
    ...
    const member = await db.prepare("SELECT 1 FROM project_members WHERE project_id = ? AND user_id = ?").get(request.params.id, request.params.userId);
    if (!member) return reply.code(404).send({ error: "Project member not found" });
    const now = new Date().toISOString();
    await db.transaction(async () => {
      await db.prepare("UPDATE tasks SET assignee_id = NULL, updated_at = ? WHERE project_id = ? AND assignee_id = ?").run(now, request.params.id, request.params.userId);
      await db.prepare("DELETE FROM project_members WHERE project_id = ? AND user_id = ?").run(request.params.id, request.params.userId);
    })();
    return reply.code(204).send();
  });

  app.delete<{ Params: ProjectParams }>("/:id", { schema: { tags: ["Projects"], summary: "Delete a project and all of its data" } }, async (request, reply) => {
    const project = await db.prepare("SELECT owner_id FROM projects WHERE id = ?").get(request.params.id) as { owner_id: string } | undefined;
    ...
    await db.prepare("DELETE FROM projects WHERE id = ?").run(request.params.id);
    return reply.code(204).send();
  });
}
