...
import { automationCopySchema, automationCreateSchema, automationUpdateSchema } from "@taskforge/contracts";
...
export async function automationRoutes(app: FastifyInstance) {
  ...
  app.post<{ Params: { projectId: string } }>("/projects/:projectId/automations/copy", async (request) => service.copy({ ...context(request), projectId: request.params.projectId }, automationCopySchema.parse(request.body)));
