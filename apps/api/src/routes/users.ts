...
export async function userRoutes(app: FastifyInstance) {
  ...
  app.get("/", { schema: { tags: ["Users"], summary: "List users and agents" } }, async () => {
    const rows = db.prepare("SELECT * FROM users ORDER BY kind, name").all() as Record<string, unknown>[];
  ...
  app.patch("/me", { schema: { tags: ["Users"], summary: "Update the current human profile" } }, async (request, reply) => {
    ...
    try {
      db.prepare("UPDATE users SET name = ?, email = ? WHERE id = ?").run(body.name, body.email.toLowerCase(), request.authUser.id);
    } catch (error) {
      if (String(error).includes("UNIQUE")) return reply.code(409).send({ error: "That email address is already in use" });
      throw error;
    }
  ...
  app.post<{ Params: UserParams }>("/:id/tokens", { schema: { tags: ["Agents"], summary: "Issue an API token (shown once)" } }, async (request, reply) => {
    ...
    db.prepare(`INSERT INTO api_tokens (id, user_id, name, token_prefix, token_hash, expires_at, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?)`)
  ...
  app.get<{ Params: UserParams }>("/:id/tokens", { schema: { tags: ["Agents"], summary: "List token metadata" } }, async (request, reply) => {
    if (request.authUser.id !== request.params.id && !requireAdmin(request, reply)) return;
    const tokens = db.prepare(`SELECT id, name, token_prefix AS prefix, expires_at AS expiresAt,
  ...
  app.delete<{ Params: TokenParams }>("/tokens/:id", { schema: { tags: ["Agents"], summary: "Revoke an API token" } }, async (request, reply) => {
    const token = db.prepare("SELECT user_id FROM api_tokens WHERE id = ?").get(request.params.id) as { user_id: string } | undefined;
    ...
    db.prepare("UPDATE api_tokens SET revoked_at = ? WHERE id = ?").run(new Date().toISOString(), request.params.id);
