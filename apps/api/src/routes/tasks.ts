...
async function recordActivity(projectId: string, taskId: string, actorId: string, action: string, metadata: unknown = {}) {
  await db.prepare("INSERT INTO activity (id, project_id, task_id, actor_id, action, metadata, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)")
    .run(randomUUID(), projectId, taskId, actorId, action, JSON.stringify(metadata), new Date().toISOString());
}

async function notify(userId: string, projectId: string, taskId: string, type: string, title: string, message: string) {
  await db.prepare(`INSERT INTO notifications (id, user_id, project_id, task_id, type, title, message, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
    .run(randomUUID(), userId, projectId, taskId, type, title, message, new Date().toISOString());
}

async function validateRelation(projectId: string, field: "assignee" | "parent", id: string | null | undefined) {
  if (!id) return true;
  if (field === "assignee") {
    return Boolean(await db.prepare("SELECT 1 FROM project_members WHERE project_id = ? AND user_id = ?").get(projectId, id));
  }
  return Boolean(await db.prepare("SELECT 1 FROM tasks WHERE project_id = ? AND id = ?").get(projectId, id));
}

async function createsParentCycle(taskId: string, parentId: string | null | undefined) {
  let cursor = parentId;
  const visited = new Set<string>();
  while (cursor) {
    if (cursor === taskId || visited.has(cursor)) return true;
    visited.add(cursor);
    const row = await db.prepare("SELECT parent_id FROM tasks WHERE id = ?").get(cursor) as { parent_id: string | null } | undefined;
...
async function replaceTaskTags(taskId: string, projectId: string, names: string[], now: string) {
  ...
  const findTag = await db.prepare("SELECT id FROM tags WHERE project_id = ? AND name = ? COLLATE NOCASE");
  const attachTag = await db.prepare("INSERT INTO task_tags (task_id, tag_id, created_at) VALUES (?, ?, ?)");
  for (const name of names) {
    await insertTag.run(randomUUID(), projectId, name, now);
    const tag = await findTag.get(projectId, name) as { id: string };
    await attachTag.run(taskId, tag.id, now);
  }
}
...
export async function taskRoutes(app: FastifyInstance) {
  ...
  app.get<{ Params: ProjectParams; Querystring: TaskQuery }>("/projects/:projectId/tasks", { schema: { tags: ["Tasks"], summary: "List project tasks" } }, async (request, reply) => {
    ...
    const rows = await db.prepare(`${taskSelect} WHERE ${where.join(" AND ")} ORDER BY t.status, t.position, t.created_at DESC`).all(...values) as Record<string, unknown>[];
    return { tasks: await Promise.all(rows.map(toTask)) };
  });
  ...
  app.post<{ Params: ProjectParams }>("/projects/:projectId/tasks", { schema: { tags: ["Tasks"], summary: "Create a task" } }, async (request, reply) => {
    ...
    await db.transaction(async () => {
      const project = await db.prepare(`SELECT next_task_number FROM projects WHERE id = ?${db.dialect === "mysql" ? " FOR UPDATE" : ""}`)
        .get(projectId) as { next_task_number: number } | undefined;
      if (!project) throw new Error("PROJECT_NOT_FOUND");
      const position = (await db.prepare("SELECT COALESCE(MAX(position), -1) + 1 AS next FROM tasks WHERE project_id = ? AND status = ?").get(projectId, body.status) as { next: number }).next;
      await db.prepare(`INSERT INTO tasks (id, project_id, number, title, description, definition_of_done, status, priority,
    ...
    const row = await db.prepare(`${taskSelect} WHERE t.id = ?`).get(id) as Record<string, unknown>;
    return reply.code(201).send({ task: await toTask(row) });
  });

  app.get<{ Params: TaskParams }>("/tasks/:id", { schema: { tags: ["Tasks"], summary: "Get a task" } }, async (request, reply) => {
    const row = await db.prepare(`${taskSelect} WHERE t.id = ?`).get(request.params.id) as Record<string, unknown> | undefined;
    if (!row) return reply.code(404).send({ error: "Task not found" });
    if (!(await requireProjectAccess(request, reply, String(row.project_id)))) return;
    return { task: await toTask(row) };
  });

  app.patch<{ Params: TaskParams }>("/tasks/:id", { schema: { tags: ["Tasks"], summary: "Update a task" } }, async (request, reply) => {
    const existing = await db.prepare("SELECT * FROM tasks WHERE id = ?").get(request.params.id) as Record<string, unknown> | undefined;
    ...
    if (fields.length || body.tags !== undefined) {
      ...
      await db.transaction(async () => {
        ...
        if (body.assigneeId && body.assigneeId !== existing.assignee_id && body.assigneeId !== request.authUser.id) {
          await notify(body.assigneeId, projectId, request.params.id, "TASK_ASSIGNED", "Task assigned to you", `${request.authUser.name} assigned you “${taskTitle}”.`);
        }
        if (body.status === "IN_REVIEW" && existing.status !== "IN_REVIEW" && existing.creator_id !== request.authUser.id) {
          await notify(String(existing.creator_id), projectId, request.params.id, "REVIEW_REQUESTED", "Review requested", `${request.authUser.name} moved “${taskTitle}” to review.`);
        }
      })();
    }
    const row = await db.prepare(`${taskSelect} WHERE t.id = ?`).get(request.params.id) as Record<string, unknown>;
    return { task: await toTask(row) };
  });
  ...
  app.get<{ Params: ProjectParams }>("/projects/:projectId/tags", { schema: { tags: ["Tasks"], summary: "List reusable project tags" } }, async (request, reply) => {
    const { projectId } = request.params;
    if (!(await requireProjectAccess(request, reply, projectId))) return;
    const tags = await db.prepare(`SELECT tags.*, COUNT(task_tags.task_id) AS task_count FROM tags
  ...
  app.get<{ Params: TaskParams }>("/tasks/:id/updates", { schema: { tags: ["Task updates"], summary: "List notes and updates on a task" } }, async (request, reply) => {
    const task = await db.prepare("SELECT project_id FROM tasks WHERE id = ?").get(request.params.id) as { project_id: string } | undefined;
    if (!task) return reply.code(404).send({ error: "Task not found" });
    if (!(await requireProjectAccess(request, reply, task.project_id))) return;
  ...
  app.post<{ Params: TaskParams }>("/tasks/:id/updates", { schema: { tags: ["Task updates"], summary: "Post a note or progress update" } }, async (request, reply) => {
    const task = await db.prepare("SELECT project_id, title, creator_id, assignee_id FROM tasks WHERE id = ?").get(request.params.id) as { project_id: string; title: string; creator_id: string; assignee_id: string | null } | undefined;
    if (!task) return reply.code(404).send({ error: "Task not found" });
    if (!(await requireProjectAccess(request, reply, task.project_id))) return;
    ...
    await db.transaction(async () => {
      ...
      await recordActivity(task.project_id, request.params.id, request.authUser.id, "task.note_added");
      ...
      for (const recipientId of recipients) await notify(recipientId, task.project_id, request.params.id, "TASK_UPDATED", "New task update", `${request.authUser.name} posted an update on “${task.title}”.`);
    })();
    const row = await db.prepare(`SELECT tu.*, u.id AS author_user_id, u.email AS author_email, u.name AS author_name,
  ...
  app.delete<{ Params: TaskParams }>("/tasks/:id", { schema: { tags: ["Tasks"], summary: "Delete a task and its subtasks" } }, async (request, reply) => {
    const existing = await db.prepare("SELECT project_id FROM tasks WHERE id = ?").get(request.params.id) as { project_id: string } | undefined;
    if (!existing) return reply.code(404).send({ error: "Task not found" });
    if (!(await requireProjectAccess(request, reply, existing.project_id))) return;
    await db.prepare("DELETE FROM tasks WHERE id = ?").run(request.params.id);
