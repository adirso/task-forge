...
export async function taskRoutes(app: FastifyInstance) {
  ...
  app.get<{ Params: ProjectParams; Querystring: TaskQuery }>("/projects/:projectId/tasks", { schema: { tags: ["Tasks"], summary: "List project tasks" } }, async (request) => {
    ...
    return { tasks: await service.list(projectContext(request, request.params.projectId), filters) };
  });
  app.post<{ Params: ProjectParams }>("/projects/:projectId/tasks", { schema: { tags: ["Tasks"], summary: "Create a task" } }, async (request, reply) => { const parsed = taskCreateSchema.safeParse(request.body); if (!parsed.success) return reply.code(400).send({ error: "Validation failed", issues: parsed.error.issues }); return reply.code(201).send({ task: await service.create(projectContext(request, request.params.projectId), parsed.data) }); });
  app.get<{ Params: TaskParams }>("/tasks/:id", { schema: { tags: ["Tasks"], summary: "Get a task" } }, async (request) => ({ task: await service.get(context(request), request.params.id) }));
  app.patch<{ Params: TaskParams }>("/tasks/:id", { schema: { tags: ["Tasks"], summary: "Update a task" } }, async (request, reply) => { const parsed = taskUpdateSchema.safeParse(request.body); if (!parsed.success) return reply.code(400).send({ error: "Validation failed", issues: parsed.error.issues }); return { task: await service.update(context(request), request.params.id, parsed.data) }; });
