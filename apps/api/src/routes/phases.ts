...
export async function phaseRoutes(app: FastifyInstance) {
  ...
  app.get<{ Params: ProjectParams }>("/projects/:projectId/phases", { schema: { tags: ["Phases"], summary: "List project phases" } }, async (request, reply) => {
    if (!(await requireProjectAccess(request, reply, request.params.projectId))) return;
    const rows = await db.prepare(`SELECT p.*, COUNT(t.id) AS task_count FROM phases p LEFT JOIN tasks t ON t.phase_id = p.id
    ...
    try {
      await db.transaction(async () => {
        if (body.isActive) {
          ...
          await db.prepare("UPDATE phases SET is_active = 0, updated_at = ? WHERE project_id = ?").run(now, projectId);
        }
        await db.prepare("INSERT INTO phases (id, project_id, number, goal, is_active, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)")
          .run(id, projectId, body.number, body.goal, body.isActive ? 1 : 0, now, now);
      })();
    } catch (error) {
      if (/UNIQUE|Duplicate entry/i.test(String(error))) return reply.code(409).send({ error: "That phase number already exists" });
      throw error;
    }
    const row = await db.prepare("SELECT *, 0 AS task_count FROM phases WHERE id = ?").get(id) as Record<string, unknown>;
  ...
  app.patch<{ Params: PhaseParams }>("/phases/:id", { schema: { tags: ["Phases"], summary: "Update or activate a phase" } }, async (request, reply) => {
    const existing = await db.prepare("SELECT * FROM phases WHERE id = ?").get(request.params.id) as Record<string, unknown> | undefined;
    ...
    try {
      await db.transaction(async () => {
        if (body.isActive) {
          if (db.dialect === "mysql") await db.prepare("SELECT id FROM projects WHERE id = ? FOR UPDATE").get(projectId);
          await db.prepare("UPDATE phases SET is_active = 0, updated_at = ? WHERE project_id = ? AND id != ?").run(now, projectId, request.params.id);
        }
        if (fields.length) { fields.push("updated_at = ?"); values.push(now, request.params.id); await db.prepare(`UPDATE phases SET ${fields.join(", ")} WHERE id = ?`).run(...values); }
      })();
    } catch (error) {
      if (/UNIQUE|Duplicate entry/i.test(String(error))) return reply.code(409).send({ error: "That phase number already exists" });
      throw error;
    }
    const row = await db.prepare("SELECT p.*, COUNT(t.id) AS task_count FROM phases p LEFT JOIN tasks t ON t.phase_id = p.id WHERE p.id = ? GROUP BY p.id").get(request.params.id) as Record<string, unknown>;
  ...
  app.delete<{ Params: PhaseParams }>("/phases/:id", { schema: { tags: ["Phases"], summary: "Delete a phase and unassign its tasks" } }, async (request, reply) => {
    const existing = await db.prepare("SELECT project_id FROM phases WHERE id = ?").get(request.params.id) as { project_id: string } | undefined;
    if (!existing) return reply.code(404).send({ error: "Phase not found" });
    if (!(await requireProjectAccess(request, reply, existing.project_id))) return;
    await db.transaction(async () => { await db.prepare("UPDATE tasks SET phase_id = NULL WHERE phase_id = ?").run(request.params.id); await db.prepare("DELETE FROM phases WHERE id = ?").run(request.params.id); })();
