  ...
  app.get<{ Querystring: ContextQuery }>("/", { schema: { tags: ["Context"], summary: "Resolve shareable project and task query parameters" } }, async (request, reply) => {
    ...
    if (taskRef) {
      if (/^[0-9a-f-]{36}$/i.test(taskRef)) {
        taskRow = await db.prepare(`${contextTaskSelect} WHERE t.id = ?`).get(taskRef) as Record<string, unknown> | undefined;
      } else {
        ...
        taskRow = await db.prepare(`${contextTaskSelect} JOIN projects task_project ON task_project.id = t.project_id
    ...
    if (projectRef) {
      projectRow = await db.prepare("SELECT * FROM projects WHERE id = ? OR `key` = ? COLLATE NOCASE").get(projectRef, projectRef) as Record<string, unknown> | undefined;
      if (!projectRow) return reply.code(404).send({ error: "Project not found" });
    } else if (taskRow) {
      projectRow = await db.prepare("SELECT * FROM projects WHERE id = ?").get(taskRow.project_id) as Record<string, unknown> | undefined;
    }
    ...
    if (!(await requireProjectAccess(request, reply, String(projectRow.id)))) return;
    return { project: toProject(projectRow), task: taskRow ? await toTask(taskRow) : null };
  });
}
