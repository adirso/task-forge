...
export class AgentRunApplicationService {
  ...
  async complete(context: RequestContext, runId: string, status: "SUCCEEDED" | "FAILED" | "CANCELLED", error?: string | null) { return this.unitOfWork.run(async (r) => { const run = await r.runs.findById(runId); if (!run) throw new NotFoundError("Agent run"); const project = await this.authorize(r, context, run.projectId); if (status === "CANCELLED" && context.actor.role !== "ADMIN" && context.actor.userId !== project.ownerId) throw new ForbiddenError("Only a project owner or administrator can cancel an agent run"); const now = this.now(); if (!(await r.runs.complete(runId, context.actor.userId, status, now, error ?? null))) throw new ValidationError("Agent run lease is not owned by this actor"); return r.runs.findById(runId); }); }
