...
export class AgentRunApplicationService {
  ...
  async list(context: RequestContext, taskId: string) {
    return this.unitOfWork.run(async (r) => { const task = await r.tasks.findById(taskId); if (!task) throw new NotFoundError("Task"); await this.authorize(r, context, task.projectId); return r.runs.listForTask(taskId); });
  }
  ...
  async heartbeat(context: RequestContext, runId: string, leaseMs = 60_000) { return this.unitOfWork.run(async (r) => { const run = await r.runs.findById(runId); if (!run) throw new NotFoundError("Agent run"); await this.authorize(r, context, run.projectId); const now = this.now(); const ok = await r.runs.heartbeat(runId, context.actor.userId, now, new Date(Date.parse(now) + Math.max(5_000, Math.min(15 * 60_000, leaseMs))).toISOString()); if (!ok) throw new ValidationError("Agent run lease is not owned by this actor"); return r.runs.findById(runId); }); }

  async complete(context: RequestContext, runId: string, status: "SUCCEEDED" | "FAILED" | "CANCELLED", error?: string | null) { return this.unitOfWork.run(async (r) => { const run = await r.runs.findById(runId); if (!run) throw new NotFoundError("Agent run"); await this.authorize(r, context, run.projectId); const now = this.now(); if (!(await r.runs.complete(runId, context.actor.userId, status, now, error ?? null))) throw new ValidationError("Agent run lease is not owned by this actor"); return r.runs.findById(runId); }); }

  private async authorize(r: RepositorySet, context: RequestContext, projectId: string) { const project = await r.projects.findById(projectId); if (!project) throw new NotFoundError("Project"); if (context.actor.role !== "ADMIN" && !(await r.memberships.isMember(projectId, context.actor.userId))) throw new ForbiddenError("You are not a member of this project"); }
}
