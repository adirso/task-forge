...
export class ProjectApplicationService implements ProjectService {
  ...
  async update(context: ProjectContext, input: Partial<Pick<ProjectEntity, "name" | "description" | "repoUrl" | "localRepoPath" | "color" | "availableStatuses" | "defaultStatus" | "agentWorkflow" | "hiddenEmptyStatuses" | "mergeTarget" | "dependencyResolutionStatuses" | "reviewPolicy">>) { return this.unitOfWork.run(async (repositories) => {
    ...
    let changes = input;
    const changesStatusConfiguration = input.availableStatuses !== undefined || input.defaultStatus !== undefined || input.agentWorkflow !== undefined || input.hiddenEmptyStatuses !== undefined || input.mergeTarget !== undefined || input.dependencyResolutionStatuses !== undefined || input.reviewPolicy !== undefined;
    if (changesStatusConfiguration && context.actor.role !== "ADMIN" && project.ownerId !== context.actor.userId) throw new ForbiddenError("Only the project owner or an administrator can manage project workflow settings");
    if (input.reviewPolicy) {
      const parsed = projectReviewPolicySchema.safeParse(input.reviewPolicy);
      if (!parsed.success) throw new ValidationError(parsed.error.issues[0]?.message ?? "Invalid review policy");
      for (const reviewerId of parsed.data.allowedReviewerAgentIds) {
        const reviewer = await repositories.users.findById(reviewerId);
        if (!reviewer || reviewer.kind !== "AGENT" || !(await repositories.memberships.isMember(project.id, reviewerId))) throw new ValidationError("Allowed reviewers must be agent members of this project");
      }
    }
