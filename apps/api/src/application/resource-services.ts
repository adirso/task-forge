...
export class ProjectApplicationService implements ProjectService {
  ...
  async update(context: ProjectContext, input: Partial<Pick<ProjectEntity, "name" | "description" | "repoUrl" | "localRepoPath" | "color" | "availableStatuses" | "defaultStatus" | "agentWorkflow" | "hiddenEmptyStatuses">>) { return this.unitOfWork.run(async (repositories) => {
    const project = await projectAccess(repositories, context, context.projectId);
    let changes = input;
    const changesStatusConfiguration = input.availableStatuses !== undefined || input.defaultStatus !== undefined || input.agentWorkflow !== undefined || input.hiddenEmptyStatuses !== undefined;
