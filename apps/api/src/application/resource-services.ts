...
export class ProjectApplicationService implements ProjectService {
  ...
  async update(context: ProjectContext, input: Partial<Pick<ProjectEntity, "name" | "description" | "repoUrl" | "color" | "availableStatuses" | "defaultStatus">>) { return this.unitOfWork.run(async (repositories) => {
    ...
    const changesStatusConfiguration = input.availableStatuses !== undefined || input.defaultStatus !== undefined;
...
export class UserApplicationService implements UserService {
  constructor(private readonly unitOfWork: UnitOfWork, private readonly now: () => string = () => new Date().toISOString(), private readonly newId: () => string = randomUUID, private readonly tokenAdapter: { create: () => { token: string; prefix: string }; hash: (token: string) => string; encrypt: (token: string) => string; decrypt: (ciphertext: string) => string } = { create: () => { throw new Error("Token issuer is not configured"); }, hash: () => { throw new Error("Token issuer is not configured"); }, encrypt: () => { throw new Error("Token encryption is not configured"); }, decrypt: () => { throw new Error("Token encryption is not configured"); } }) {}
