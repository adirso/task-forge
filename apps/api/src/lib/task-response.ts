...
export function taskResponse(task: TaskEntity): ContractTask {
  return {
    ...task,
    tags: task.tags ?? [],
    dependencies: (task.dependencies ?? []).map((dependency) => ({ ...dependency, projectKey: dependency.projectKey ?? "", isBlocking: dependency.isBlocking ?? (dependency.status !== "DONE" && dependency.status !== "CANCELLED") })),
