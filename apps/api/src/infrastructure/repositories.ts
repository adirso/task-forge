...
function createAgentHandoffRepository(db: DatabasePort): AgentHandoffRepository { return { async findByRun(runId) { const row = await db.prepare("SELECT * FROM agent_run_handoffs WHERE run_id = ?").get(runId); return row ? toAgentHandoff(row) : null; }, async save(input) { const exists = await db.prepare("SELECT run_id FROM agent_run_handoffs WHERE run_id = ?").get(input.runId); const values = [input.taskId, input.branch, input.headSha, input.branchPublished ? 1 : 0, input.pullRequestUrl, input.pullRequestTitle, input.pullRequestState, input.status, input.lastError, input.createdAt, input.updatedAt, input.runId]; if (exists) await db.prepare("UPDATE agent_run_handoffs SET task_id=?, branch=?, head_sha=?, branch_published=?, pull_request_url=?, pull_request_title=?, pull_request_state=?, status=?, last_error=?, created_at=?, updated_at=? WHERE run_id=?").run(...values); else await db.prepare("INSERT INTO agent_run_handoffs (task_id,branch,head_sha,branch_published,pull_request_url,pull_request_title,pull_request_state,status,last_error,created_at,updated_at,run_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)").run(...values); return input; } }; }
