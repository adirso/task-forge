...
function toProject(row: Row): ProjectEntity {
  ...
  return { id: text(row.id), key: text(row.key), name: text(row.name), description: text(row.description), repoUrl: nullableText(row.repo_url), localRepoPath: nullableText(row.local_repo_path), color: text(row.color), sortOrder: Number(row.sort_order ?? 0), availableStatuses, defaultStatus, ownerId: text(row.owner_id), createdAt: date(row.created_at), updatedAt: date(row.updated_at), ...(row.task_count !== undefined ? { taskCount: Number(row.task_count) } : {}) };
}
