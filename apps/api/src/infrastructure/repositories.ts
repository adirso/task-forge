...
function toAgentRun(row: Record<string, unknown>): AgentRunEntity {
  return { id: text(row.id), taskId: text(row.task_id), projectId: text(row.project_id), requestedById: text(row.requested_by_id), executedById: nullableText(row.executed_by_id), kind: row.kind as AgentRunEntity["kind"], status: row.status as AgentRunEntity["status"], attemptCount: Number(row.attempt_count), maxAttempts: Number(row.max_attempts), leaseOwner: nullableText(row.lease_owner), leaseExpiresAt: nullableText(row.lease_expires_at), heartbeatAt: nullableText(row.heartbeat_at), timeoutAt: nullableText(row.timeout_at), lastError: nullableText(row.last_error), createdAt: date(row.created_at), updatedAt: date(row.updated_at), completedAt: nullableText(row.completed_at) };
}
...
function createAgentRunRepository(db: DatabasePort): AgentRunRepository {
  ...
  return {
    async create(input) { await db.prepare("INSERT INTO agent_runs (id, task_id, project_id, requested_by_id, executed_by_id, kind, status, attempt_count, max_attempts, lease_owner, lease_expires_at, heartbeat_at, timeout_at, last_error, created_at, updated_at, completed_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").run(input.id, input.taskId, input.projectId, input.requestedById, input.executedById, input.kind, input.status, input.attemptCount, input.maxAttempts, input.leaseOwner, input.leaseExpiresAt, input.heartbeatAt, input.timeoutAt, input.lastError, input.createdAt, input.updatedAt, input.completedAt); return input; },
    ...
    async claim(id, owner, now, leaseExpiresAt) { return Boolean((await db.prepare("UPDATE agent_runs SET status = 'RUNNING', attempt_count = attempt_count + 1, executed_by_id = ?, lease_owner = ?, lease_expires_at = ?, heartbeat_at = ?, updated_at = ? WHERE id = ? AND status IN ('PENDING', 'FAILED') AND attempt_count < max_attempts").run(owner, owner, leaseExpiresAt, now, now, id)).changes); },
...
function toTaskGate(row: Row, approvals: TaskGateEntity["approvals"] = []): TaskGateEntity {
  const parse = (value: unknown) => { try { return JSON.parse(String(value ?? "[]")); } catch { return []; } };
  return { taskId: text(row.task_id), headSha: text(row.head_sha), requiredChecks: parse(row.required_checks) as string[], checks: parse(row.checks_json) as TaskGateEntity["checks"], implementationRunId: nullableText(row.implementation_run_id), implementationAgentId: nullableText(row.implementation_agent_id), approvals, approvedHeadSha: nullableText(row.approved_head_sha), approvedById: nullableText(row.approved_by_id), approvedAt: nullableText(row.approved_at), mergedHeadSha: nullableText(row.merged_head_sha), mergedById: nullableText(row.merged_by_id), mergedAt: nullableText(row.merged_at), updatedAt: text(row.updated_at) };
}
