...
async function migrate(adapter: Adapter, dialect: DatabaseDriver) {
  ...
  if (dialect === "mysql") {
    const column = await adapter.get<{ count: number }>("SELECT COUNT(*) AS count FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name = 'projects' AND column_name = 'sort_order'", []);
    if (!Number(column?.count)) await adapter.run("ALTER TABLE projects ADD COLUMN sort_order INT NOT NULL DEFAULT 0, ADD INDEX idx_projects_sort_order (sort_order)", []);
  }
  if (dialect === "sqlite") {
