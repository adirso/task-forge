...
import { AutomationEngine } from "../src/application/automation-service.js";
...
test("automation handoff failures are audited and remain visible to the caller", async () => {
  ...
  const repositories = {
    ...
    activity: { record: async (event: { action: string; metadata: Record<string, unknown> }) => { events.push(event); } },
  } as unknown as RepositorySet;

  await assert.rejects(() => new AutomationEngine().apply(repositories, context, task, task, "TASK_UPDATED"), /database unavailable/);
  assert.equal(events[0]?.action, "automation.failed");
  assert.deepEqual(events[0], { projectId: "project-1", taskId: "task-1", actorId: "agent-1", action: "automation.failed", metadata: { automationId: "automation-1", trigger: "TASK_UPDATED", error: "database unavailable" } });
});
