...
function setup() {
  let finding: any = null; const runs: any[] = []; const activities: any[] = []; const gateState: { value: any } = { value: null };
  const set = { users: { findById: async (id: string) => ({ id }) }, projects: { findById: async () => project }, memberships: { isMember: async () => true }, tasks: { findById: async () => task, update: async (_id: string, input: unknown) => ({ ...task, ...input }) },
    ...
    runs: { findById: async () => null, countForTask: async () => runs.length, create: async (run: any) => { runs.push(run); return run; } }, gates: { findByTask: async () => gateState.value, save: async (input: any) => { gateState.value = input; return input; } }, activity: { record: async (input: any) => { activities.push(input); } },
  } as unknown as RepositorySet;
  return { service: new TaskFindingApplicationService({ run: async (work) => work(set) }, () => "2026-08-24T12:00:00.000Z", () => "00000000-0000-4000-8000-000000000001"), runs, activities, gateState, set };
}

test("finding dispositions are audited and FIX_NEEDED creates a new fix run", async () => {
  const { service, runs, activities, gateState } = setup();
