...
const project = { id: "project-1", key: "TAS", name: "Task Forge", repoUrl: null, ownerId: "owner-1", mergeTarget: "main", availableStatuses: ["IN_REVIEW", "FIX_NEEDED", "IN_PROGRESS", "PENDING_DECISION"] } as never;
...
function setup() {
  let finding: any = null; const runs: any[] = []; const contextPacks: any[] = []; const activities: any[] = []; const deliveries: any[] = []; const gateState: { value: any } = { value: null }; let invalidatedApprovals = 0;
  const set = { users: { findById: async (id: string) => id === "agent-1" ? { id, kind: "AGENT", webhookUrl: "https://agent.example/webhook" } : { id } }, projects: { findById: async () => project }, memberships: { isMember: async () => true }, tasks: { findById: async () => task, update: async (_id: string, input: unknown) => ({ ...task, ...input }) },
    ...
    runs: { findById: async () => null, listForTask: async () => runs, countForTask: async () => runs.length, cycleState: async () => ({ count: runs.length, limit: 6, limitFailure: false, failureEventId: null }), create: async (run: any) => { runs.push(run); return run; } }, contextPacks: { findCurrent: async () => contextPacks.at(-1) ?? null, nextVersion: async () => contextPacks.length + 1, create: async (pack: any) => { contextPacks.push(pack); const run = runs.find((candidate) => candidate.id === pack.runId); if (run) { run.contextPackVersion = pack.version; run.contextPackFingerprint = pack.fingerprint; } return pack; } }, updates: { listForTask: async () => ({ items: [], page: { hasMore: false, nextCursor: null } }) }, gates: { findByTask: async () => gateState.value, save: async (input: any) => { gateState.value = input; return input; }, invalidateApprovals: async () => { invalidatedApprovals += 1; if (gateState.value) gateState.value = { ...gateState.value, approvals: [] }; } }, activity: { record: async (input: any) => { activities.push(input); } }, webhookDeliveries: { create: async (input: any) => { deliveries.push(input); return input; } },
...
test("finding dispositions are audited and FIX_NEEDED creates a new fix run", async () => {
  ...
  gateState.value = { taskId: task.id, headSha: "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", requiredChecks: ["Quality"], requiredArtifactTypes: [], checks: [], implementationRunId: "run-1", implementationAgentId: "agent-1", approvals: [{ reviewerId: "codex", approvedAt: "2026-08-24T11:00:00.000Z" }], approvedHeadSha: "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", approvedById: "codex", approvedAt: "2026-08-24T11:00:00.000Z", mergedHeadSha: "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", mergedById: "owner", mergedAt: "2026-08-24T11:30:00.000Z", updatedAt: "2026-08-24T11:30:00.000Z" };
