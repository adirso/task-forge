...
const codex = { actor: { userId: "codex-1", kind: "AGENT" as const, role: "MEMBER" as const, name: "Codex Reviewer", tokenScopes: null } };
...
test("merge requires Codex approval and human authorization, then records merged evidence", async () => {
  ...
  await assert.rejects(() => service.approve(human, task.id, head), /Codex/);
