...
export default function App() {
  ...
  async function updateProject(input: { name: string; description: string; repoUrl: string | null; localRepoPath: string | null; color: string; availableStatuses: TaskStatus[]; defaultStatus: TaskStatus; agentWorkflow?: import("@taskforge/contracts").AgentWorkflow | null; hiddenEmptyStatuses?: TaskStatus[]; mergeTarget?: "main" | "phase"; dependencyResolutionStatuses?: import("@taskforge/contracts").DependencyResolutionStatus[] }) {
    if (!currentProject) return;
  ...
  async function enableAgentWorkflow() {
    if (!currentProject) return;
    const { project } = await api.enableAgentWorkflow(currentProject.id);
    setCurrentProject((current) => current?.id === project.id ? { ...current, ...project } : current);
    setProjects((items) => items.map((item) => item.id === project.id ? { ...item, ...project } : item));
    flash("Agent workflow enabled");
  }
            ...
            <div className="project-title-row">
              ...
              <div className="header-actions">
                ...
                <ProjectHeaderActions
                  ...
                  onCreateTask={() => setNewTaskStatus(currentProject.defaultStatus)}
                />
              </div>
            </div>
