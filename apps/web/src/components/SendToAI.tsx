  ...
  return (
    <div className="ai-dialog-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <section className="send-to-ai-dialog" role="dialog" aria-modal="true" aria-labelledby="send-to-ai-title">
        <header><div><span className="ai-dialog-kicker"><Sparkles /> Agent handoff</span><h2 id="send-to-ai-title">{mode === "IMPLEMENT" ? "Implement" : "Review"} {project.key}-{task.number} with AI</h2><p>{mode === "IMPLEMENT" ? "Give an agent a complete, provider-specific implementation brief." : "Give an agent a complete, evidence-driven review brief."}</p></div><button type="button" className="icon-button" onClick={onClose} aria-label="Close AI handoff"><X /></button></header>
        <div className="ai-mode-tabs" role="tablist" aria-label="AI task mode"><button type="button" role="tab" aria-selected={mode === "IMPLEMENT"} className={mode === "IMPLEMENT" ? "selected" : ""} onClick={() => { setMode("IMPLEMENT"); setCopied(null); }}>Implement</button><button type="button" role="tab" aria-selected={mode === "REVIEW"} className={mode === "REVIEW" ? "selected" : ""} onClick={() => { setMode("REVIEW"); setCopied(null); }}>Review</button></div>
        ...
        <footer><button type="button" className="button button-secondary" onClick={() => copy(contextUrl, "link")}><ExternalLink /> {copied === "link" ? "Link copied" : "Copy task link"}</button><div><button type="button" className="button button-secondary" onClick={onClose}>Cancel</button><button type="button" className="button button-primary ai-copy-button" onClick={() => copy(prompt, "prompt")}>{copied === "prompt" ? <Check /> : <Clipboard />}{copied === "prompt" ? "Prompt copied" : `Copy ${mode === "IMPLEMENT" ? "implementation" : "review"} prompt`}</button></div></footer>
      </section>
    </div>
  );
}
