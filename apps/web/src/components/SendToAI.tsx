...
import { aiProviders, buildAIPrompt, buildTaskContextUrl, type AIProvider } from "../lib/aiPrompt";

export function SendToAI({ project, task, phaseNumber, onClose }: { project: Project; task: Task; phaseNumber: number | null; onClose: () => void }) {
  ...
  const prompt = useMemo(() => buildAIPrompt({ provider, project, task, phaseNumber, contextUrl, apiBaseUrl }), [provider, project, task, phaseNumber, contextUrl, apiBaseUrl]);
  ...
  return (
    <div className="ai-dialog-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <section className="send-to-ai-dialog" role="dialog" aria-modal="true" aria-labelledby="send-to-ai-title">
        <header><div><span className="ai-dialog-kicker"><Sparkles /> Agent handoff</span><h2 id="send-to-ai-title">Send {project.key}-{task.number} to AI</h2><p>Choose an agent and copy a complete, provider-specific implementation prompt.</p></div><button type="button" className="icon-button" onClick={onClose} aria-label="Close Send to AI"><X /></button></header>
        ...
        <div className="ai-security-note"><ShieldCheck /><span>No TaskForge token or JWT is included. The agent is instructed to use its own configured credential.</span></div>
        {error && <div className="form-error">{error}</div>}
        <footer><button type="button" className="button button-secondary" onClick={() => copy(contextUrl, "link")}><ExternalLink /> {copied === "link" ? "Link copied" : "Copy task link"}</button><div><button type="button" className="button button-secondary" onClick={onClose}>Cancel</button><button type="button" className="button button-primary ai-copy-button" onClick={() => copy(prompt, "prompt")}>{copied === "prompt" ? <Check /> : <Clipboard />}{copied === "prompt" ? "Prompt copied" : `Copy for ${selectedProvider.name}`}</button></div></footer>
      </section>
    </div>
  );
}
