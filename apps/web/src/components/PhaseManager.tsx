...
export function PhasesPage({ project, phases, onChange }: { project: Project; phases: Phase[]; onChange: (phases: Phase[]) => void }) {
  ...
  return (
    <div className="phases-page">
      ...
      <div className="phases-page-layout"><section><div className="phase-page-section-title"><h3>All phases</h3><span>{project.name}</span></div><div className="phase-list">{phases.map((phase) => <article key={phase.id} className={phase.isActive ? "active" : ""}><span className="phase-number">{phase.number}</span><div><span><strong>Phase {phase.number}</strong>{phase.isActive && <em><CheckCircle2 /> Active</em>}</span><p>{phase.goal}</p><small>{phase.nonDoneTaskCount ?? 0} non-done · {phase.completedTaskCount ?? 0} done · {phase.cancelledTaskCount ?? 0} cancelled</small></div><div>{!phase.isActive && <button className="button button-secondary" onClick={() => activate(phase.id)}>Set active</button>}<button className="phase-delete" onClick={() => remove(phase)} aria-label={`Delete Phase ${phase.number}`}><Trash2 /></button></div></article>)}</div>{!phases.length && <div className="phases-empty"><Flag /><strong>No phases yet</strong><span>Create the first phase using the form.</span></div>}</section>
