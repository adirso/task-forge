  ...
  return (
    <div className="modal-backdrop" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <form className="task-modal" role="dialog" aria-modal="true" aria-labelledby="task-modal-title" onSubmit={submit}>
        <header><div><span className="modal-kicker">{task ? `${project.key}-${task.number}` : `New task in ${project.name}`}</span><h2 id="task-modal-title">{task ? "Edit task" : "Create a task"}</h2></div><div className="modal-header-actions">{task && <div className="ai-action-buttons"><button type="button" className="send-to-ai-button" onClick={() => setAIMode("IMPLEMENT")}><Sparkles /> Implement</button><button type="button" className="send-to-ai-button review" onClick={() => setAIMode("REVIEW")}><ShieldCheck /> Review</button><button type="button" className="send-to-ai-button fix" onClick={() => setAIMode("FIX")}><WrenchIcon /> Fix needed</button><button type="button" className="send-to-ai-button rereview" onClick={() => setAIMode("RE_REVIEW")}><RefreshCwIcon /> Re-review</button></div>}{task && <button type="button" className="copy-task-link" onClick={() => copyTaskLink().catch(() => setError("Could not copy task link"))}>{linkCopied ? <Check /> : <Link2 />}{linkCopied ? "Copied" : "Copy link"}</button>}<button type="button" className="icon-button" onClick={onClose}><X /></button></div></header>
