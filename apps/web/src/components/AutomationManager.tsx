...
const operatorLabels: Record<(typeof operators)[number], string> = {
  ...
  changed_from_to: "Moved from → to",
  is_empty: "Is empty",
  is_not_empty: "Is not empty",
};
const services = ["agent-api", "github-actions", "taskforge"];
const blankCondition = (): AutomationCondition => ({ field: "status", operator: "equals", value: "", fromValue: null });
const blankAction = (status: TaskStatus = "IN_PROGRESS"): AutomationAction => ({ field: "status", valueType: "static", value: status });
...
export function AutomationManager({ project, users, phases = [] }: { project: Project | null; users: User[]; phases?: Phase[] }) {
  ...
  function valueControl(field: AutomationCondition["field"], value: string | null, onChange: (value: string) => void, placeholder = "Value") {
    const options = field === "status" ? project?.availableStatuses.map((status) => ({ value: status, label: statusMeta[status].label })) ?? [] : choices[field] ?? (field === "assigneeId" ? users.map((user) => ({ value: user.id, label: user.name })) : field === "phaseId" ? phases.map((phase) => ({ value: phase.id, label: `Phase ${phase.number}` })) : null);
    if (options) return <select value={value ?? ""} onChange={(event) => onChange(event.target.value)}><option value="">Choose {fields.find((item) => item.value === field)?.label.toLowerCase()}</option>{options.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select>;
  ...
  return <div className="automation-page">
    ...
    <div className="automation-layout"><section className="automation-card automation-builder"><div className="automation-card-heading"><div><span className="eyebrow">Rule builder</span><h3>{editing ? "Edit automation" : "Create an automation"}</h3></div><span className="step-badge">{editing ? "Editing" : "New rule"}</span></div><label>Rule name<input value={name} onChange={(e) => setName(e.target.value)} placeholder="Assign task to actioner" /></label><div className="automation-two-col"><label>When<select value={trigger} onChange={(e) => setTrigger(e.target.value as AutomationCreate["trigger"])}><option value="TASK_UPDATED">A task is updated</option><option value="TASK_CREATED">A task is created</option></select></label><label>Triggered by<select value={actorType} onChange={(e) => setActorType(e.target.value as AutomationCreate["actorType"])}><option value="ANY">Anyone</option><option value="USER">A specific user</option><option value="SERVICE">A service</option></select></label></div>{actorType === "USER" && <label>Specific user<select value={actorId ?? ""} onChange={(e) => setActorId(e.target.value)}><option value="">Choose a user</option>{users.map((u) => <option key={u.id} value={u.id}>{u.name}</option>)}</select></label>}{actorType === "SERVICE" && <label>Service<select value={service ?? ""} onChange={(e) => setService(e.target.value)}><option value="">Choose a service</option>{services.map((s) => <option key={s}>{s}</option>)}</select></label>}
      <div className="automation-section-title"><span>Conditions</span><small>All conditions must match</small></div>
      {conditions.map((condition, index) => {
        const isTransition = condition.operator === "changed_from_to";
        return (
          <div className={`automation-rule-row${isTransition ? " is-transition" : ""}`} key={index}>
            <select aria-label="Condition field" value={condition.field} onChange={(e) => updateCondition(index, { field: e.target.value as AutomationCondition["field"], value: "", fromValue: null })}>
              {fields.map((field) => <option key={field.value} value={field.value}>{field.label}</option>)}
            </select>
            <select aria-label="Condition operator" value={condition.operator} onChange={(e) => updateCondition(index, { operator: e.target.value as AutomationCondition["operator"], fromValue: e.target.value === "changed_from_to" ? condition.fromValue ?? "" : null })}>
              {operators.map((operator) => <option key={operator} value={operator}>{operatorLabels[operator]}</option>)}
            </select>
            {isTransition ? (
              <div className="automation-transition-values">
                <label><span>From</span>{valueControl(condition.field, condition.fromValue ?? "", (value) => updateCondition(index, { fromValue: value }), "Previous value")}</label>
                <span className="automation-transition-arrow" aria-hidden="true">→</span>
                <label><span>To</span>{valueControl(condition.field, condition.value, (value) => updateCondition(index, { value }), "New value")}</label>
              </div>
            ) : !condition.operator.includes("empty") ? (
              valueControl(condition.field, condition.value, (value) => updateCondition(index, { value }))
            ) : <span />}
            <button type="button" className="automation-remove" onClick={() => setConditions((items) => items.filter((_, i) => i !== index))} aria-label="Remove condition"><Trash2 /></button>
          </div>
        );
      })}
