...
export function ProjectDashboard({ project, tasks, phases }: { project: Project; tasks: Task[]; phases: Phase[] }) {
  ...
  useEffect(() => { let active = true; void api.deliveryMonitorHealth().then((result) => { if (active) setMonitor(result.monitor); }).catch(() => { if (active) setMonitor(null); }); return () => { active = false; }; }, []);
