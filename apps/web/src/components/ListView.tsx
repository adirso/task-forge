      ...
      {groups.map((group) => <section className={`phase-table-section ${group.phase?.isActive ? "is-active" : ""}`} key={group.phase?.id ?? "unassigned"}>
        ...
        <div className="list-shell">
          <table className="task-table">
            <thead><tr><th>Task</th><th>Status</th><th>Assignee</th><th>Priority</th><th>Pull request</th><th>Due date</th><th>Points</th></tr></thead>
            ...
            )) : <tr className="empty-phase-row"><td colSpan={7}>No tasks match this phase and the current filters.</td></tr>}</tbody>
          </table>
        </div>
      </section>)}
    </div>
