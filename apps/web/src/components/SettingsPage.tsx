...
import { Activity, Bot, Check, Copy, Eye, KeyRound, LayoutDashboard, List, Monitor, Plus, Save, ShieldCheck, Trash2, UserRound, Webhook } from "lucide-react";
...
export function SettingsPage({ user, users, defaultView, textSize, onUserUpdated, onAgentCreated, onAgentUpdated, onAgentDeleted, onDefaultViewChange, onTextSizeChange }: {
  ...
  async function saveWebhook(event: FormEvent) {
    event.preventDefault(); if (!selectedAgentId) return;
    setSavingWebhook(true); setError("");
    try {
      const { user: updated } = await api.updateAgentWebhook(selectedAgentId, webhookUrl.trim() || null);
      onAgentUpdated(updated); success("Webhook URL saved");
    } catch (err) { setError(err instanceof Error ? err.message : "Could not save webhook URL"); }
    ...
    <div className="settings-page">
      ...
      <div className="settings-layout">
        <nav className="settings-nav">
          ...
          {user.role === "ADMIN" && <button className={tab === "agentops" ? "active" : ""} onClick={() => setTab("agentops")}><Activity /> Agent ops</button>}
        </nav>
