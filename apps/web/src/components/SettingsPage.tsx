...
import { Bot, Check, Copy, KeyRound, LayoutDashboard, List, Monitor, Plus, ShieldCheck, Trash2, UserRound } from "lucide-react";
import { api } from "../lib/api";
import { Avatar } from "./Avatar";

type SettingsTab = "account" | "appearance" | "agents";

export function SettingsPage({ user, users, defaultView, textSize, onUserUpdated, onAgentCreated, onDefaultViewChange, onTextSizeChange }: {
  user: User; users: User[]; defaultView: "board" | "list"; textSize: "comfortable" | "large";
  onUserUpdated: (user: User) => void; onAgentCreated: (user: User) => void;
  onDefaultViewChange: (view: "board" | "list") => void; onTextSizeChange: (size: "comfortable" | "large") => void;
}) {
  ...
  return (
    <div className="settings-page">
      <header className="settings-header"><span>Workspace</span><h1>Settings</h1><p>Manage your account, preferred workspace view, and agent access.</p></header>
      <div className="settings-layout">
        <nav className="settings-nav"><button className={tab === "account" ? "active" : ""} onClick={() => setTab("account")}><UserRound /> Account</button><button className={tab === "appearance" ? "active" : ""} onClick={() => setTab("appearance")}><Monitor /> Appearance</button><button className={tab === "agents" ? "active" : ""} onClick={() => setTab("agents")}><Bot /> Agents <span>{agents.length}</span></button></nav>
