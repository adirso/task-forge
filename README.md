...
- Move a workspace between computers with a secure backup: `npm run backup:create -w @taskforge/api -- --output ./backups/taskforge-$(date +%Y%m%d-%H%M%S).tar.gz --include-secrets`. This contains the full database and task attachments; MySQL backups include every table in the selected database, including binary fields. Copy the archive securely, configure the new computer with the same database driver and a compatible schema, then restore with `npm run backup:restore -w @taskforge/api -- --input ./backups/taskforge-YYYYMMDD-HHMMSS.tar.gz --force`. Secure archives contain password hashes, API tokens, and webhook credentials, so protect them like production credentials. The archive does not contain application source, `.env` configuration, Git repositories, or Smithy worktrees; move or clone those separately and recreate machine-specific settings. Run restore only against the intended target database and attachment directory.
...
Smithy's loopback-only `POST /jobs/:eventId/cancel` control endpoint is disabled unless `SMITHY_CANCEL_SECRET` is configured (at least 32 characters). Authorized callers send the exact JSON body `{"eventId":"..."}` and `X-TaskForge-Signature` using the same timestamped HMAC-SHA256 format as agent webhooks (`t=<unix-seconds>,v1=<hex HMAC of "<timestamp>.<exact-body>">`). Requests without a valid, fresh signature are rejected, so a cross-site browser form cannot cancel jobs. Keep this secret in the operator's private environment file and share it only with trusted control clients.
